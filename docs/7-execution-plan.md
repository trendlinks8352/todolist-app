...
- ☐ 의존성 설치: `express`, `pg`, `jsonwebtoken`, `bcrypt`, `joi`, `dotenv`, `cors`
- ☐ 개발 의존성 설치: `jest`, `supertest`, `nodemon`
- ☐ 디렉토리 구조 생성: `src/routes/`, `src/controllers/`, `src/services/`, `src/repositories/`, `src/middlewares/`, `src/config/`, `src/db/`, `src/utils/`, `src/types/`, `src/constants/`, `src/auth/strategies/`, `test/integration/`
...
- ☐ `DELETE /api/users/me` 후 todos·사용자 정의 categories 삭제 확인
- ☐ 미인증 요청 → 401
...
#### 세부 작업
- ☐ `jest.config.js` 설정, `.env.test` 별도 테스트 DB 환경변수
- ☐ `test/helpers/setup.js` — beforeAll: DB 연결·seed, afterAll: pool 종료·테스트 데이터 정리
- ☐ **인증 테스트** (`test/integration/auth.test.js`)
  - ☐ UC-01: 회원가입 성공 → 201 + `accessToken`
  - ☐ UC-01: 중복 이메일 → 409, 비밀번호 정책 위반 → 422
  - ☐ UC-02: 로그인 성공 → 200, 잘못된 비밀번호 → 401
  - ☐ UC-03: 로그아웃 → 200
  - ☐ BR-01: 토큰 없이 `/api/todos` → 401
- ☐ **할일 테스트** (`test/integration/todo.test.js`)
  - ☐ UC-05: 등록 성공 → 201, `categoryId` 누락 → 422, 과거 dueDate → 422
  - ☐ UC-07: 카테고리·완료여부·기간 필터 조합 조회
  - ☐ UC-06: 수정 성공, BR-03 타인 수정 → 403
  - ☐ UC-08: 완료 처리 → `completedAt` 기록, 취소 → null
  - ☐ UC-12: 삭제 → 204, BR-03 타인 삭제 → 403
- ☐ **카테고리 테스트** (`test/integration/category.test.js`)
  - ☐ 목록 — 기본 3개 포함, UC-09 생성 → 201, BR-10 중복 → 409
- ☐ **사용자 테스트** (`test/integration/user.test.js`)
  - ☐ `GET /api/users/me` 성공·`password` 미포함
  - ☐ UC-13: `DELETE /api/users/me` → 204, todos CASCADE 삭제 확인

#### 완료 조건
- ☐ `npm test` 전체 통과
- ☐ BR-03 소유권 검증 전 케이스 통과
- ☐ BR-06 completedAt 자동 처리 테스트 통과
- ☐ DC-01 CASCADE 삭제 검증 통과
- ☐ 테스트 후 DB 잔여 데이터 없음
...
- ☐ 두 화면: `isAuthenticated` 시 `/todos` 자동 리다이렉트
...
*작성일: 2026-05-12 | 버전: v1.0 | 참조: PRD v1.2, 도메인 정의서 v1.2, ERD v1.0*
